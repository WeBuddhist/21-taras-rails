...
def main():
    ...
    for f in maps:
        ...
        extra = [c for c in table_rows(mtxt, ['Extra claims']) if clean(c[0]) == slot]
        ...
        extra_ids, amb_ids = [], []
        for cells in extra:
            for cid in CLAIM_ID.findall(clean(cells[1] if len(cells) > 1 else '')):
                if cid in blocks and cid not in picked:
                    extra_ids.append(cid)
        for cells in amb:
            for cid in CLAIM_ID.findall(clean(cells[0])):
        ...
        for cid in extra_ids:
            why = ''
            for cells in extra:
                if cid in clean(cells[1] if len(cells) > 1 else ''):
                    why = clean(cells[2]) if len(cells) > 2 else ''
            body.append(f"> ⓘ **Extra claim** — sits outside this slot's nodes. {why}".rstrip())
            body.append(blocks[cid])
            body.append('')
            manifest.append(f"{rid}:{cid}")
        for cid in amb_ids:
