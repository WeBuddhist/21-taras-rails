...
Two optional flags, added 2026-08-01 (see Provenance); both default off, so the
behaviour above is unchanged unless you ask for them:

- `--incipit` — also accept a **one-line citation of the opening pāda inside a
  `ཞེས`/`ཅེས` frame**, on the same line: `ཕྱག་འཚལ་སྟོན་ཀའི་ཟླ་བ་ཀུན་ཏུ།། ཞེས་པ་ནི།`.
  This is the normal citation shape in a `བསྟོད་འགྲེལ`, where the commentator quotes
  the incipit and expounds rather than reproducing the stanza. Scored below a real
  multi-pāda match, so a fuller quotation elsewhere in the file still wins.
- `--in-order` — refuse a placement that sits above the previous verse's. A
  commentary follows its root text in order, so a candidate earlier in the file is
  a recurrence of the phrase, not this verse's citation.
...
Ported verbatim from `bodhisattvacharyavatara-rails`, where the three stages are
in production use across ten Bodhicaryāvatāra commentaries.

### Changes on 2026-08-01 — `01_transclude_verses.py`

Ingesting the tara21 corpus (21-verse Tārā praise, 16 commentaries) placed
**116 of 352** possible anchors, with six commentaries anchoring *nothing*. Three
causes, all specific to assumptions the Bodhicaryāvatāra corpus happened to
satisfy:

1. **`is_closer()` compared Tibetan against Latin transliterations** (`'zespa ni'`,
   `'cespani'`) and so could never return `True`. The single-line-citation branch
   that calls it was dead code. Now checks the real normalised closers
   (`ཞེས`, `ཅེས`, `སོགས`).
2. **A blank line counted as a mismatch** in `stanza_score()`. Commentaries that
   set each pāda of a quoted stanza as its own block have blank lines *between*
   the pādas, which exhausted the one-miss budget after the first pāda: a fully
   quoted stanza scored 1 of 4. Blank lines are now skipped — an empty string can
   never `line_match` anything, so this cannot create a match that was not there.
   **Unconditional**, being a plain correctness fix.
3. **No way to match an incipit citation.** Added behind `--incipit`, with
   `--in-order` alongside it. Both default off.

Measured after the change:

| corpus | before | after |
|---|---:|---:|
| tara21, 16 commentaries × 22 verses | 116 (33%) | **209 (59%)** |
| spyodjug `BCAC14_GDR`, vault anchors stripped, no flags | 775 | **782** |

The spyodjug figure is the regression check: on a file where the vault's own
human-checked anchors are the ground truth, the new script places seven *more*
verses, loses none, moves one, and every placement is a verse the vault also
anchors. Change 2 is what produces that gain.

The six tara21 commentaries that anchored nothing now anchor 19, 17, 16, 19, 21
and 18 of 22. The three still at zero (`TARAC10_DPN`, `TARAC11_KMT`,
`TARAC14_LZD`) are the word-commentaries — `བསྡུས་འགྲེལ` and `མཆན་འགྲེལ` — which
dissolve the stanza into glosses and genuinely never quote it. That is the
structural limit `stages/align.py` documents, not a tuning problem; those files
fall through to the cluster matcher and the human checkpoint.
