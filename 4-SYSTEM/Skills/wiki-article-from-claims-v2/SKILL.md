...
description: Draft a readable, encyclopedic Tibetan Wikipedia article from one consolidated claims topic page — wikivoice prose, at most 3 refs per statement, at most 2 commentary quotations per article — plus a generated read-only footnote preview for reviewers; same claim-resolution chain and quotation verification as wiki-article-from-claims.
---
...
4. **Commentary metadata** — from each raw claims file's own frontmatter (`author`, `title`, `author_in_english`/`title_in_english` where present). Never from memory.
...
Rules 1–4 and 10–14 are v1's machinery, unchanged. Rules 5–9 are this skill's reason to exist — the style delta.
...
2. **Build the claim-resolution table** — exactly as v1: every attestation ID on the consolidated page resolved through its raw tree-guided file to བོད་ཡིག, English gloss, `Cite:` path + block ID, and author/title frontmatter. Record unresolvables before drafting a single sentence.
...
4. **Outline, then draft.** Map the consolidated page's sections onto the skeleton. Draft the lead (bold name, identification, one–two cited sentences). Draft each body section as connected wikivoice prose per Rules 5–8, attaching ≤3 refs per statement; put attributed material where Rule 5 sends it.
...
3. **Rewrite for register.** Merge one-claim-one-sentence sequences into connected wikivoice prose (Rules 5, 8). The rewrite may only use claim IDs that already appear in the source `citations.md` — introducing a new claim ID in Mode B is not permitted; that requires Mode A.
...
7. **Readability pass (Rule 9).** Reread start to finish; redraft any remaining who-said-what fragment sequence outside the divergence section.
