...
1. **Full vault-relative paths only.** Every transclusion link uses the full path from the vault root with the `.md` extension, e.g. `![[1-SOURCES/Text/bo-སྒྲོལ་མ་ཉེར་གཅིག་ལ་བསྟོད་པ།.md#^1-1]]` — never a bare note name or short wiki-link, per the vault-wide convention in `transclusion` Rule 2.
