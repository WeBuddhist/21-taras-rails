...
> skip straight from the format step to `add-toc` / `root-text-frontmatter`.
...
`texts/<text-id>/work/` and is **not** yet `annotated.md`; human review is
...
```
  texts/<text-id>/work/segmented.md
   ...
   5. Render:     assign block IDs from depth, insert headings, wrap wikilinks by anchored
       ...
       ▼
  texts/<text-id>/work/tagged-segmented.md
```
...
| Field | Description |
|---|---|
| Input file | Path to a formatted text file — typically `texts/<text-id>/work/segmented.md` |
...
A single file at `texts/<text-id>/work/tagged-<original-filename>` (the
...
```json
{
  "source_file": "texts/<text-id>/work/segmented.md",
  "candidates": [
    {
      "line": 4999,
      "type": "enumeration",
...
`texts/<text-id>/work/<filename>.annotation.json`.
...
python3 skills/tag-inline-toc/scripts/tag_inline_toc.py render \
    ...
    --annot  texts/<text-id>/work/segmented.md.annotation.json \
...
python3 skills/tag-inline-toc/scripts/tag_inline_toc.py verify \
    --input <input-file> --tagged texts/<text-id>/work/tagged-segmented.md
```

Then report to the user: the render counts, the output path, and every node
you marked `"review"`. The output stays in `texts/<text-id>/work/` — it is
...
8. **Output goes to `texts/<text-id>/work/`**, never overwrites `raw.md` or `annotated.md` directly.
...
- [ ] Output written to `texts/<text-id>/work/tagged-<filename>`; review nodes reported to the user
...
Paths changed from `0-INBOX/segmentation/` / `0-INBOX/temp/` to
`texts/<text-id>/work/`; `tag_inline_toc.py`'s default output-path helper
(`derive_output_path`) changed from a hardcoded `0-INBOX/temp` constant to
