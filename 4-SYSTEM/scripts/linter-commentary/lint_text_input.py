#!/usr/bin/env python3
"""Lint commentary source documents against the API schemas.

Resolves commentary_of from root_text → text_id, validates edition/TOC,
and writes a structured JSON payload.

On success: writes output/<stem>.lint.json
On failure: writes output/<stem>.lint.errors.json

Usage:
    python lint_text_input.py path/to/commentary.md
"""
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Validate commentary document against the TextInput / Edition schema."
    )
    ...
    for path in args.paths:
        ...
        if file_type and file_type != "commentary":
            print(
                f"ERROR {path}: linter-commentary expects file_type 'commentary', got {file_type!r}",
                file=sys.stderr,
            )
            had_errors = True
            continue
                ...
                try:
                    ...
                    if root_info:
                        ...
                        if root_file_type:
                            ...
                            print(
                                f"  WARN root_text {resolved_root.name}: file_type not set",
                                file=sys.stderr,
                            )
                        if not doc_info.get("commentary_of"):
                            commentary_of = root_info.get("text_id")
                            if commentary_of and str(commentary_of).strip():
                                doc_info["commentary_of"] = str(commentary_of).strip()
                            else:
                                print(
                                    f"  WARN root_text {resolved_root.name}: "
                                    "no text_id found to set commentary_of",
                                    file=sys.stderr,
                                )
                    ...
                    print(
                        f"  WARN root_text: could not read {resolved_root}: {exc}",
                        file=sys.stderr,
                    )
            else:
                print(
                    f"  WARN root_text: {root_text_val!r} could not be resolved to a file",
                    file=sys.stderr,
                )
