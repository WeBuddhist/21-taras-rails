    ...
    for raw in blocks:
        ...
        if not is_header and _ref_part_count(ref) > VERSE_REF_MAX_PARTS:
            items.append(("ERROR",
                ...
                f"content segments allow at most ^n-n-n ({VERSE_REF_MAX_PARTS} parts)"))
            continue
...
def validate_edition(data, body):
    ...
    if not data.get("file_type"):
        items.append(("ERROR", "file_type: required; expected 'commentary'"))
    elif data.get("file_type") != "commentary":
        items.append(("ERROR",
            f"file_type: expected 'commentary', got {data.get('file_type')!r}"))
...
def validate_text_input(data):
    ...
    for field in ("wiki", "date", "translation_of"):
        if field in data:
            validate_nullable_string(data[field], field, items)
        else:
            items.append(("INFO", f"{field}: not set (optional)"))

    # commentary_of links this work to the root text's text_id (for API + alignment)
    commentary_of = data.get("commentary_of")
    if commentary_of and str(commentary_of).strip():
        validate_nullable_string(commentary_of, "commentary_of", items)
        items.append(("INFO", f"commentary_of: {commentary_of!r}"))
    else:
        items.append(("WARN",
            "commentary_of: not set — resolve from root_text's text_id "
            "(set text_id on the root text first)"))

    if not data.get("root_text"):
        items.append(("ERROR", "root_text: required for file_type 'commentary'"))
    else:
