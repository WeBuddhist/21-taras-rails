...
| `bo-སྒྲོལ་མ་ཉེར་གཅིག་ལ་བསྟོད་པ།-en.md` | The rendered translation, block-ID aligned to the source. |
...
python3 4-SYSTEM/Skills/dharmamitra-translate/scripts/dm_translate.py \
  --source "1-SOURCES/Text/bo-སྒྲོལ་མ་ཉེར་གཅིག་ལ་བསྟོད་པ།.md" --lang english
```
...
| Date | Step | Result |
| --- | --- | --- |
| 2026-08-27 | Produced in `Liturgy-rails` by DharmaMitra `cat-translate` against the flat block ids `^1`–`^29` of the earlier cut of the same critical edition; uploaded to the library and aligned 1:1 | 29/29 blocks, line-parallel to the source |
...
| 2026-09-17 | **Uploaded** to library.webuddhist.com under the existing text `W5o6Tyq3hhQDxhmvdoS7B`: edition `BtqPpZvVCamzWXhjObYka` (32 segments), alignment root→translation (32 identity pairs), TOC `Lkvj4CoCjpiOmJPsWd5Ls` (1 root section + 4 subsections); read back and diffed against the payloads | live |
| 2026-09-21 | The five sibling zero-shot tracks (DharmaMitra zh; Gemini hi, mn, ne, vi) were **withdrawn**: alignment, TOC, edition and text deleted from the library (their text ids no longer resolve) and the track folders removed from the vault, because zero-shot output has no vocabulary standardisation and no per-segment fact-check. This English baseline is the only translation left in the vault and the library. Plan: rebuild every language through `graded-translate` (keyword termbase → term-locked translation → drift check) and `commentary-fact-check` before any re-upload; a re-upload creates a new text id | this track kept |
