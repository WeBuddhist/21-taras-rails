#!/usr/bin/env bash
# Thai draft 2 — Gemini run primed with the locked Thai word list (keyword-standardize).
# Run on your Mac, in Terminal:   bash 3-TRANSFORMATIONS/Translations/Gemini/th-general/run-th-general.sh
...
OUT="3-TRANSFORMATIONS/Translations/Gemini/th-general"
...
# Pick the strongest Gemini Pro model this key can use (override with MODEL=... bash run-th-general.sh).
if [ -z "${MODEL:-}" ]; then
...
python3 "$GM" --source "$SRC" --lang thai --lang-tag th --out "$OUT" --model "$MODEL" --thinking high
python3 "$GM" --source "$SRC" --lang thai --lang-tag th --out "$OUT" --model "$MODEL" --thinking high --headings
echo
echo "Done. Output: $OUT/bo-སྒྲོལ་མ་ཉེར་གཅིག་ལ་བསྟོད་པ།-th.md — tell Claude it has finished."
