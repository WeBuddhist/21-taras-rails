#!/usr/bin/env bash
# Thai draft 1 — Gemini zero-shot (no word list), the baseline and second opinion for the Thai word list.
# Run on your Mac, in Terminal:   bash 3-TRANSFORMATIONS/Translations/Gemini/th/run-th.sh
...
OUT="3-TRANSFORMATIONS/Translations/Gemini/th"
...
# Pick the strongest Gemini Pro model this key can use (override with MODEL=... bash run-th.sh).
if [ -z "${MODEL:-}" ]; then
