...
| 2026-09-17 | Re-translated with the same generator under the track's `style.md`: `^2-6` (the added optative ཅིག) and `^1-1` (reading བྱེ་བ→ཕྱེ་བ; the imported rendering had 'ten million' stamens). The older record stays in the ledger; the newer one renders | see `work/*.jsonl` |
...
| 2026-09-17 | **Uploaded** to library.webuddhist.com under the existing text `QGjmqg2hVtIjqJdifTVUV`: edition `8PKpBCH7ylnmOBDwArpzb` (32 segments), alignment root→translation (32 identity pairs), TOC `EvLJD78ez2U6S36NKQ7oO` (1 root section + 4 subsections); read back and diffed against the payloads | live |
