---
title: "hi — Gemini zero-shot (hindi)"
...
| `work/<text>-hi.jsonl` | Append-only ledger, one per source text: one record per block, holding source, translation, the exact context sent, model version, token usage, line-parity result and timings. The audit trail and the resume point. |
| `<text>-hi.md` | The rendered translation, block-ID aligned to the source. |
...
python3 4-SYSTEM/Skills/gemini-translate/scripts/gm_translate.py \
  --source "1-SOURCES/Text/<text>.md" --lang hindi
python3 4-SYSTEM/Skills/gemini-translate/scripts/gm_translate.py \
  --source "1-SOURCES/Text/<text>.md" --lang hindi --headings     # section headings
```
...
| Date | Step | Result |
| --- | --- | --- |
| 2026-09-07 | Produced in `Liturgy-rails` by Gemini `gemini-3.1-pro-preview` against the flat block ids `^1`–`^29` of the earlier cut of the same critical edition; uploaded to the library and aligned 1:1 | 29/29 blocks, line-parallel to the source |
...
| 2026-09-17 | **Uploaded** to library.webuddhist.com under the existing text `4NECafzLDkznK7jBxfmHU`: edition `ZzYzbThBTiUGmzyq66hAZ` (32 segments), alignment root→translation (32 identity pairs), TOC `BR4it8Nco2pI86y98CmXS` (1 root section + 4 subsections); read back and diffed against the payloads | live |
