---
topic: tathagata
article: article.md
method: wiki-article-from-claims
context_packages:
  - 2-RAILS/Claims/tathagata.md
rails_status: draft
raw_sources_cited:
  - 2-RAILS/Claims/raw/tree-guided/anon-trinle-char.md
  - 2-RAILS/Claims/raw/tree-guided/anon-utpala.md
  - 2-RAILS/Claims/raw/tree-guided/drakpa-gyaltsen.md
  - 2-RAILS/Claims/raw/tree-guided/gendun-drub.md
  - 2-RAILS/Claims/raw/tree-guided/gendun-gyatso.md
  - 2-RAILS/Claims/raw/tree-guided/karma-maitri.md
  - 2-RAILS/Claims/raw/tree-guided/konchok-thabkhe.md
  - 2-RAILS/Claims/raw/tree-guided/lobsang-dawa.md
  - 2-RAILS/Claims/raw/tree-guided/palden-sherab.md
  - 2-RAILS/Claims/raw/tree-guided/pema-namgyal.md
  - 2-RAILS/Claims/raw/tree-guided/sangye-nyentrul.md
  - 2-RAILS/Claims/raw/tree-guided/sungrab-tulku.md
  - 2-RAILS/Claims/raw/tree-guided/taranatha.md
  - 2-RAILS/Claims/raw/tree-guided/tenga-tulku.md
  - 2-RAILS/Claims/raw/tree-guided/tenzin-dhonzang.md
  - 2-RAILS/Claims/raw/tree-guided/tsultrim-namdak.md
date: 2026-08-14
status: draft
---

# Citations — tathagata (དེ་བཞིན་གཤེགས་པ)

Scope: this article covers *Tathāgata* (དེ་བཞིན་གཤེགས་པ) as a generic buddha-epithet in
this corpus — its etymology, its dominant use in the root text's fourth homage verse
(the goddess as the Tathāgatas' crown-ornament / uṣṇīṣa), named individual buddhas
called "Tathāgata" (Vairocana, Amitābha, Dundhubhisvara/Drumsound), tantric
class-term uses, and one commentary's own completion-stage doctrinal reading — as
attested by all sixteen commentaries per `2-RAILS/Claims/tathagata.md`. It deliberately
excludes the parallel epithet རྒྱལ་བ ("Victor"), which is the sibling topic
`2-RAILS/Claims/conqueror.md` and its own article, even where the two notions occur in
the same commentarial sentence.

## Reference map

Ref names are shared per commentary (Rule 7): the same `<ref name="X">` is defined once,
in full, on first use, and reused as `<ref name="X" />` on every later citation of that
commentary elsewhere in the article — regardless of which claim/block that later citation
supports. The table below lists one row per citation event (i.e. per claim actually
drawn on), even where several rows share the same ref name.

| Ref | Commentary | Claim ID | Quotation (verbatim བོད་ཡིག, if quoted) | Source block |
|---|---|---|---|---|
| sungrab-tulku | sungrab-tulku | c-7-2 | — (lead: paraphrased) | 1-SOURCES/Commentaries/སྒྲོལ་མཉེར་གཅིག་གི་རྣམ་བཤད།.md#^0-34 |
| anon-utpala | anon-utpala | c-1-2-2-1-1-3-3 | — (lead: paraphrased) | 1-SOURCES/Commentaries/སྒྲོལ་མར་ཕྱག་འཚལ་ཉེར་གཅིག་གིས་བསྟོད་པའི་རྣམ་བཤད་ཡིད་འཕྲོག་ཨུཏྤལའི་ཆུན་པོ་ཞེས་བྱ་བ་བཞུགས་སོ།.md#^0-23 |
| taranatha | taranatha | c-4-9 | "དཔལ་ལྡན་དེ་བཞིན་ཉིད་གཤེགས་ཤིང་། ། དེ་བཞིན་སླར་ཡང་གཤེགས་པ་ཉིད" | 1-SOURCES/Commentaries/ཕྱག་འཚལ་ཉེར་གཅིག་གི་བསྟོད་པའི་རྣམ་པར་བཤད་པ།.md#^0-16 |
| tenzin-dhonzang | tenzin-dhonzang | c-4-4-3 | "ཆོས་རྣམས་ཀྱི་ཆོས་ཉིད་དེ་བཞིན་ཉིད་སྟོང་པ་ཉིད་ཀྱི་དབྱིངས་ལ་ཡེ་ཤེས་ཀྱིས་དེ་དང་རྗེས་སུ་མཐུན་པར་གཤེགས་ཤིང་མཉམ་པར་བཞག་པས་ན། དེ་བཞིན་གཤེགས་པའི་མཚན་གསོལ་བ་ཡིན" | 1-SOURCES/Commentaries/སྒྲོལ་མ་ཉེར་གཅིག་གི་བསྟོད་འགྲེལ་སྙིང་གི་ནོར་བུ.md#^0-132 |
| tenzin-dhonzang | tenzin-dhonzang | c-4-4-4 | "ཇི་སྐད་སྨྲས་པ་དེ་བཞིན་བྱེད" | #^0-133 |
| anon-trinle-char | anon-trinle-char | c-3-4-3 / c-3-4-4 | — (paraphrased: verse gloss "most-used" claim) | 1-SOURCES/Commentaries/སྒྲོལ་མའི་འགྲེལ་བ་འཕྲིན་ལས་ཆར་དུ་སྙིལ་བའི་སྤྲིན་ཕུང་།.md#^0-41, #^0-42 |
| anon-utpala | anon-utpala | c-1-2-2-1-1-3-2 | — (shares scope note above; not separately quoted) | #^0-22 |
| drakpa-gyaltsen | drakpa-gyaltsen | c-1-23 | — (paraphrased) | 1-SOURCES/Commentaries/སྒྲོལ་མ་ཕྱག་འཚལ་ཉི་ཤུ་རྩ་གཅིག་གི་བསྟོད་པའི་རྣམ་བཤད་གསལ་བའི་འོད་ཟེར་ཞེས་བྱ་བ་བཞུགས་སོ།.md#^0-24 |
| gendun-drub | gendun-drub | c-2-2-2-2-1-1-3-2 | — (paraphrased) | 1-SOURCES/Commentaries/སྒྲོལ་མ་ཕྱག་འཚལ་ཉེར་གཅིག་གི་ཊཱིཀྐ་རིན་པོ་ཆེའི་ཕྲེང་བ།.md#^0-29 |
| gendun-gyatso | gendun-gyatso | c-1-4-1 | "དེ་བཞིན་གཤེགས་པ་ཐམས་ཅད་ཀྱིས་ཀྱང། གཙུག་ཏོར་ལྟར་མཆོད་པའི་གནས་སུ་སྒྱུར" | 1-SOURCES/Commentaries/ཕྱག་འཚལ་སྒྲོལ་མ་ཉེར་གཅིག་མའི་རྣམ་བཤད།.md#^0-12 |
| karma-maitri | karma-maitri | c-1-1-4-2 | — (paraphrased) | 1-SOURCES/Commentaries/ཕྱག་འཚལ་སྒྲོལ་མ་ཉེར་གཅིག་གི་བསྟོད་པའི་བསྡུས་འགྲེལ།.md#^0-10 |
| konchok-thabkhe | konchok-thabkhe | c-2-4-2 | — (used twice: below, quoted) | 1-SOURCES/Commentaries/ཕྱག་འཚལ་ཉེར་གཅིག་མའི་ཊིཀྐ་འཕགས་མའི་ཞལ་ལུང་ཞེས་བྱ་བ་བཞུགས་སོ།.md#^0-28 |
| lobsang-dawa | lobsang-dawa | c-1-2-2-1-1-3-2 | — (paraphrased) | 1-SOURCES/Commentaries/སྒྲོལ་མ་ཕྱག་འཚལ་ཉེར་གཅིག་གི་མཆན་འགྲེལ།.md#^0-10 |
| palden-sherab | palden-sherab | c-3-1-4-0-2 | — (paraphrased) | 1-SOURCES/Commentaries/རྗེ་བཙུན་སྒྲོལ་མའི་བསྟོད་པ་ཉི་ཤུ་རྩ་གཅིག་གི་ཚིག་དོན་རྣམ་པར་འགྲེལ་བ་དད་བརྩོན་བྱང་ཆུབ་སེམས་མཆོག་གི་པདྨའི་གཞོན་ནུ་ཁ་འབྱེད་པའི་ཐབས་ཤེས་ཉི་ཟླའི་འཛུམ་རླབས་ཞེས་བྱ་བཞུགས་སོ།.md#^0-44 |
| pema-namgyal | pema-namgyal | c-2-4-20 | — (paraphrased) | 1-SOURCES/Commentaries/ཕྱག་འཚལ་ཉེར་གཅིག་གི་བསྟོད་འགྲེལ་བདུད་རྩིའི་དགའ་ཚལ་བཞུགས་སོ།།.md#^0-34 |
| sangye-nyentrul | sangye-nyentrul | c-5-0-1 | — (paraphrased) | 1-SOURCES/Commentaries/རྗེ་བཙུན་མ་འཕགས་མ་སྒྲོལ་མ་ཉི་ཤུ་རྩ་གཅིག་གི་ཚིག་འགྲེལ་དང་དམིགས་རིམ་ཉུང་ངུར་བཀོད་པ་འཕགས་མའི་བྱིན་རླབས་གྲུ་ཆར་བཞུགས།།.md#^0-19 |
| sungrab-tulku | sungrab-tulku | c-7-2 | "ཕྱག་འཚལ་དེ་བཞིན་གཤེགས་པའི་གཙུག་ཏོར། མཐའ་ཡས་རྣམ་པར་རྒྱལ་བར་སྤྱོད་མ༎ མ་ལུས་ཕ་རོལ་ཕྱིན་པ་ཐོབ་པའི།། རྒྱལ་བའི་སྲས་ཀྱིས་ཤིན་ཏུ་བསྟེན་མ།། ཞེས་པ་སྟེ" | #^0-34 |
| gendun-gyatso | gendun-gyatso | c-1-4-1 | "དེ་བཞིན་གཤེགས་པ་ཐམས་ཅད་ཀྱིས་ཀྱང། གཙུག་ཏོར་ལྟར་མཆོད་པའི་གནས་སུ་སྒྱུར" (repeat, second use) | #^0-12 |
| anon-utpala | anon-utpala | c-1-2-2-1-1-3-3 | "དེ་བཞིན་གཤེགས་པ་ཐམས་ཅད་ཀྱི་ཡུམ་ཡིན་པས་དེ་རྣམས་ཀྱི་དབུའི་གཙུག་ཏོར་གྱི་རྩེ་ན" | #^0-23 |
| gendun-drub | gendun-drub | c-2-2-2-2-1-1-3-3 | "དེ་བཞིན་གཤེགས་པ་ཐམས་ཅད་ཀྱི་ཡུམ་ཡིན་པས་དབུའི་གཙུག་ཏོར་ལྟར་འཁུར" | #^0-29 |
| gendun-drub | gendun-drub | c-0-5 | "དེ་བཞིན་གཤེགས་པ་ཐམས་ཅད་ཀྱི་ཡུམ་སྒྲོལ་མ་ལ་བསྟོད་པ" | #^0-3 |
| konchok-thabkhe | konchok-thabkhe | c-2-4-2 | "དེ་བཞིན་གཤེགས་པ་ཐམས་ཅད་ཀྱིས་ཀྱང་གཙུག་ཏོར་ལྟར་བཀུར་བའི་འོས་སུ་གྱུར་པ" | #^0-28 |
| tsultrim-namdak | tsultrim-namdak | c-3-3 | "དེ་བཞིན་གཤེགས་པ་རྣམ་པར་སྣང་མཛད་ཀྱི་ཞལ་ལས་ གསུངས་པའི་བསྟོད་པ" | 1-SOURCES/Commentaries/སྒྲོལ་འགྲེལ་ཚོགས་གཉིས་རྒྱ་མཚོར་འཇུག་པའི་གྲུ་གཟིངས།.md#^0-177 |
| taranatha | taranatha | c-4-3 | "དེ་བཞིན་གཤེགས་པ་རྣམ་པར་སྣང་མཛད་ཀྱི་གཙུག་གཏོར་ལས" | 1-SOURCES/Commentaries/ཕྱག་འཚལ་ཉེར་གཅིག་གི་བསྟོད་པའི་རྣམ་པར་བཤད་པ།.md#^0-13 |
| anon-trinle-char | anon-trinle-char | c-3-4-7 | "དེ་བཞིན་གཤེགས་པ་རྣམ་པ་སྣང་མཛད་ཀྱི་དབུའི་གཙུག་ཏོར་ལས་རིག་སྔགས་ཀྱི་ཡེ་ཤེས་སྤྲུལ་པ" | 1-SOURCES/Commentaries/སྒྲོལ་མའི་འགྲེལ་བ་འཕྲིན་ལས་ཆར་དུ་སྙིལ་བའི་སྤྲིན་ཕུང་།.md#^0-45 |
| anon-trinle-char | anon-trinle-char | c-3-4-8 | — (paraphrased: Geluk scholars' generic reading) | #^0-45 |
| taranatha | taranatha | c-12-7 | "རལ་པའི་ཐོར་ཚུགས་འཆར་བའི་ཁྲོད་ན་དེ་བཞིན་གཤེགས་པ་འོད་དཔག་ཏུ་མེད་པ་བཞུགས་ཤིང་གནས་པ" | #^0-41 |
| taranatha | taranatha | c-21-4-16 | "དེ་བཞིན་གཤེགས་པའི་གཙུག་ཏོར་ཞེས་པས་ལྷ་དབང་བསྐུར" | #^0-83 |
| tenzin-dhonzang | tenzin-dhonzang | c-4-12-9 | "དེ་བཞིན་གཤེགས་པ་བརྒྱད" | #^0-210 |
| tenzin-dhonzang | tenzin-dhonzang | c-4-17-14 | "དེ་བཞིན་གཤེགས་པའི་གཙུག་ཏོར་གྱིས་ཀྱང་གཟིགས་སོ། ། དེ་བཞིན་མཛོད་སྤུས་ཀྱང་གཟིགས་སོ" | #^0-258 |
| tsultrim-namdak | tsultrim-namdak | c-3-24 | "དེ་བཞིན་གཤེགས་པ་ཐམས་ཅད་ཀྱི་སྐུ་གསུང་ཐུགས་ཀྱི་ས་ བོན" | #^0-261–#^0-266 |
| sungrab-tulku | sungrab-tulku | c-2-1 | — (paraphrased) | 1-SOURCES/Commentaries/སྒྲོལ་མཉེར་གཅིག་གི་རྣམ་བཤད།.md#^0-8 |
| tenzin-dhonzang | tenzin-dhonzang | c-1-1 | — (paraphrased) | 1-SOURCES/Commentaries/སྒྲོལ་མ་ཉེར་གཅིག་གི་བསྟོད་འགྲེལ་སྙིང་གི་ནོར་བུ.md#^0-7 |
| tsultrim-namdak | tsultrim-namdak | c-2-1-1-9 | — (paraphrased) | 1-SOURCES/Commentaries/སྒྲོལ་འགྲེལ་ཚོགས་གཉིས་རྒྱ་མཚོར་འཇུག་པའི་གྲུ་གཟིངས།.md#^0-66 |
| sungrab-tulku | sungrab-tulku | c-2-1 | "སྔོན་གྱི་དུས་སུ་བཅོམ་ལྡན་འདས་དེ་བཞིན་གཤེགས་པ་རྔ་སྒྲ་ཞེས་བྱ་བ་བྱུང་སྟེ" | #^0-8 |
| tenzin-dhonzang | tenzin-dhonzang | c-1-1 | "སྔོན་ཐོག་མ་མེད་པའི་དུས་ཤིག་ན། འཇིག་རྟེན་གྱི་ཁམས་སྣ་ཚོགས་པའི་འོད་ཅེས་བྱ་བར། དེ་བཞིན་གཤེགས་པ་རྔ་སྒྲ་ཞེས་བྱ་བ་བྱུང་སྟེ" | #^0-7 |
| sungrab-tulku | sungrab-tulku | c-2-1 | — (paraphrased: Yeshe Dawa's devotion, shares block ^0-8 with c-2-6) | #^0-8 |
| tenzin-dhonzang | tenzin-dhonzang | c-1-1 | — (paraphrased) | #^0-7 |
| sungrab-tulku | sungrab-tulku | c-2-6 | "ཇི་སྲིད་བླ་ན་མེད་པའི་བྱང་ཆུབ་མངོན་དུ་བྱས་པ་དེ་སྲིད་དུ་མིང་ལྷ་མོ་སྒྲོལ་མའི་མཚན་འབའ་ཞིག་ཏུ་འགྱུར་བར" | #^0-8 |
| tenzin-dhonzang | tenzin-dhonzang | c-1-6 | — (shares same wording as c-2-6, same block ^0-8 as parent claim c-1-1) | #^0-7 |
| tsultrim-namdak | tsultrim-namdak | c-2-1-1-9 | — (shares same prophecy content) | #^0-66 |
| anon-trinle-char | anon-trinle-char | c-3-4-7 / c-3-4-8 | — (paraphrased: internal divergence framing) | #^0-45 |
| palden-sherab | palden-sherab | c-3-1-4-3-1 | "དེ་བཞིན་གཤེགས་པ་ནི་དགའ་བཞི་ཡེ་ཤེས" | #^0-50 |
| palden-sherab | palden-sherab | c-3-1-4-4-1 | "དེ་བཞིན་གཤེགས་པའི་གཙུག་ཏོར་ནི་ཐེག་པ་རིམ་དགུའི་རྩེ་མོ་རྫོགས་ཆེན་ཨ་ཏི་ཡོ་ག་ལྟ་བ" | #^0-51 |
| sungrab-tulku | sungrab-tulku | c-2-1 / c-2-6 (summary) | — (paraphrased, bsdus-don) | #^0-8 |
| tenzin-dhonzang | tenzin-dhonzang | c-1-1 / c-1-6 (summary) | — (paraphrased, bsdus-don) | #^0-7 |

## Claims used but not quoted

Several claims entered the prose as paraphrase rather than direct quotation — every
such sentence still carries its own `<ref>`. Representative examples: `sungrab-tulku:c-7-2`
and `anon-utpala:c-1-2-2-1-1-3-3` (lead identification), `anon-trinle-char:c-3-4-3`/`c-3-4-4`
(fourth-homage "most-used" framing), `drakpa-gyaltsen:c-1-23`, `gendun-drub:c-2-2-2-2-1-1-3-2`,
`karma-maitri:c-1-1-4-2`, `lobsang-dawa:c-1-2-2-1-1-3-2`, `palden-sherab:c-3-1-4-0-2`,
`pema-namgyal:c-2-4-20`, `sangye-nyentrul:c-5-0-1` (fourth-homage consensus, un-quoted
attestations kept as stacked refs per Rule 5's "backbone cited to representative
commentaries" guidance rather than quoted individually), `anon-trinle-char:c-3-4-8`
(Geluk generic-reading note), `sungrab-tulku:c-2-1`/`tenzin-dhonzang:c-1-1` (origin-story
narrative detail beyond the two quoted sentences), `tsultrim-namdak:c-2-1-1-9` (prophecy,
same content as the quoted `sungrab-tulku:c-2-6`/`tenzin-dhonzang:c-1-6`).

## Claims resolved but not incorporated

The consolidated page's "Excluded"-adjacent material and several Consensus attestations
were resolved against their raw files but not separately quoted, in the interest of
keeping the article to at most a few representative verbatim quotations per section
(Rule 3) rather than a citation chain. This applies to the remaining fourth-homage
Consensus attestations not individually quoted above (`gendun-gyatso:c-1-4-1` is quoted;
the rest of the sixteen are cited via stacked `<ref>` only). No divergent or unique
position is thereby lost — every Consensus, Divergence, and Unique claim on
`2-RAILS/Claims/tathagata.md` is represented in the article's prose, cited, even where
not directly quoted.

## Unresolvable attestations

None. Every attestation ID drawn on from `2-RAILS/Claims/tathagata.md` resolved cleanly
against its raw tree-guided file.

## Warnings

- **`rails_status: draft`.** `2-RAILS/Claims/tathagata.md` carries `status: draft` in its
  own frontmatter (per this vault's rule, an LLM never marks its own consolidation
  `complete`). This article was drafted from it anyway on the same accepted-risk basis as
  the sibling `mantra` and `conqueror` articles: the consolidated page passed
  `verify_consolidation.py` with 0 errors and 0 warnings (45 unique citations). A human
  contributor should set `2-RAILS/Claims/tathagata.md`'s own `status` to `complete` before
  this article is treated as final.
- **No public URLs exist for any of the sixteen commentaries.** Per the wikitext spec's
  hand-formatted ref form, every `<ref>` in this article carries only author and title —
  no URL, no year, no page number — because none of these commentaries has a page/URL
  recorded anywhere upstream (their raw claims files' own frontmatter carries no `year`
  or `page` field). This applies uniformly to all sixteen refs.
- **Scope boundary against `conqueror.md`.** རྒྱལ་བ ("Victor") material that co-occurs with
  དེ་བཞིན་གཤེགས་པ in the same commentarial sentence (e.g. the fourth homage's own root
  verse, which contains both "Homage to the Tathāgatas' uṣṇīṣa" and "relied upon by the
  Victors' sons") is deliberately not re-drafted here for its རྒྱལ་བ content — see
  `3-TRANSFORMATIONS/Wikipedia/tara21/term-articles/conqueror/article.md` for that
  material's own article.
- **Article length.** The drafted body runs to roughly 1,300–1,500 Tibetan syllables by
  rough count, close to the spec's ~1,500-syllable warning threshold. This is noted, not
  treated as blocking (warnings are non-blocking per V-rule table).

## Verification

Every quotation was located character-for-character (whitespace-collapsed) in the
`1-SOURCES/` file its claim's `Cite:` target names, checked directly against the
`1-SOURCES/Commentaries/` files (not merely against the raw claims file's own
`**བོད་ཡིག:**` field) per Rule 4.

| # | Quotation (as drafted) | Claim | Result |
|---|---|---|---|
| 1 | "དཔལ་ལྡན་དེ་བཞིན་ཉིད་གཤེགས་ཤིང་། ། དེ་བཞིན་སླར་ཡང་གཤེགས་པ་ཉིད" | taranatha:c-4-9 | PASS |
| 2 | "ཆོས་རྣམས་ཀྱི་ཆོས་ཉིད་དེ་བཞིན་ཉིད་སྟོང་པ་ཉིད་ཀྱི་དབྱིངས་ལ་ཡེ་ཤེས་ཀྱིས་དེ་དང་རྗེས་སུ་མཐུན་པར་གཤེགས་ཤིང་མཉམ་པར་བཞག་པས་ན། དེ་བཞིན་གཤེགས་པའི་མཚན་གསོལ་བ་ཡིན" | tenzin-dhonzang:c-4-4-3 | PASS |
| 3 | "ཇི་སྐད་སྨྲས་པ་དེ་བཞིན་བྱེད" | tenzin-dhonzang:c-4-4-4 | PASS |
| 4 | "ཕྱག་འཚལ་དེ་བཞིན་གཤེགས་པའི་གཙུག་ཏོར། མཐའ་ཡས་རྣམ་པར་རྒྱལ་བར་སྤྱོད་མ༎ མ་ལུས་ཕ་རོལ་ཕྱིན་པ་ཐོབ་པའི།། རྒྱལ་བའི་སྲས་ཀྱིས་ཤིན་ཏུ་བསྟེན་མ།། ཞེས་པ་སྟེ" | sungrab-tulku:c-7-2 | PASS |
| 5 | "དེ་བཞིན་གཤེགས་པ་ཐམས་ཅད་ཀྱིས་ཀྱང། གཙུག་ཏོར་ལྟར་མཆོད་པའི་གནས་སུ་སྒྱུར" | gendun-gyatso:c-1-4-1 | PASS |
| 6 | "དེ་བཞིན་གཤེགས་པ་ཐམས་ཅད་ཀྱི་ཡུམ་ཡིན་པས་དེ་རྣམས་ཀྱི་དབུའི་གཙུག་ཏོར་གྱི་རྩེ་ན" | anon-utpala:c-1-2-2-1-1-3-3 | PASS |
| 7 | "དེ་བཞིན་གཤེགས་པ་ཐམས་ཅད་ཀྱི་ཡུམ་ཡིན་པས་དབུའི་གཙུག་ཏོར་ལྟར་འཁུར" | gendun-drub:c-2-2-2-2-1-1-3-3 | PASS |
| 8 | "དེ་བཞིན་གཤེགས་པ་ཐམས་ཅད་ཀྱི་ཡུམ་སྒྲོལ་མ་ལ་བསྟོད་པ" | gendun-drub:c-0-5 | PASS |
| 9 | "དེ་བཞིན་གཤེགས་པ་ཐམས་ཅད་ཀྱིས་ཀྱང་གཙུག་ཏོར་ལྟར་བཀུར་བའི་འོས་སུ་གྱུར་པ" | konchok-thabkhe:c-2-4-2 | PASS |
| 10 | "དེ་བཞིན་གཤེགས་པ་རྣམ་པར་སྣང་མཛད་ཀྱི་ཞལ་ལས་ གསུངས་པའི་བསྟོད་པ" | tsultrim-namdak:c-3-3 | PASS |
| 11 | "དེ་བཞིན་གཤེགས་པ་རྣམ་པར་སྣང་མཛད་ཀྱི་གཙུག་གཏོར་ལས" | taranatha:c-4-3 | PASS |
| 12 | "དེ་བཞིན་གཤེགས་པ་རྣམ་པ་སྣང་མཛད་ཀྱི་དབུའི་གཙུག་ཏོར་ལས་རིག་སྔགས་ཀྱི་ཡེ་ཤེས་སྤྲུལ་པ" | anon-trinle-char:c-3-4-7 | PASS |
| 13 | "རལ་པའི་ཐོར་ཚུགས་འཆར་བའི་ཁྲོད་ན་དེ་བཞིན་གཤེགས་པ་འོད་དཔག་ཏུ་མེད་པ་བཞུགས་ཤིང་གནས་པ" | taranatha:c-12-7 | PASS |
| 14 | "དེ་བཞིན་གཤེགས་པའི་གཙུག་ཏོར་ཞེས་པས་ལྷ་དབང་བསྐུར" | taranatha:c-21-4-16 | PASS |
| 15 | "དེ་བཞིན་གཤེགས་པ་བརྒྱད" | tenzin-dhonzang:c-4-12-9 | PASS |
| 16 | "དེ་བཞིན་གཤེགས་པའི་གཙུག་ཏོར་གྱིས་ཀྱང་གཟིགས་སོ། ། དེ་བཞིན་མཛོད་སྤུས་ཀྱང་གཟིགས་སོ" | tenzin-dhonzang:c-4-17-14 | PASS |
| 17 | "དེ་བཞིན་གཤེགས་པ་ཐམས་ཅད་ཀྱི་སྐུ་གསུང་ཐུགས་ཀྱི་ས་ བོན" | tsultrim-namdak:c-3-24 | PASS |
| 18 | "སྔོན་གྱི་དུས་སུ་བཅོམ་ལྡན་འདས་དེ་བཞིན་གཤེགས་པ་རྔ་སྒྲ་ཞེས་བྱ་བ་བྱུང་སྟེ" | sungrab-tulku:c-2-1 | PASS |
| 19 | "སྔོན་ཐོག་མ་མེད་པའི་དུས་ཤིག་ན། འཇིག་རྟེན་གྱི་ཁམས་སྣ་ཚོགས་པའི་འོད་ཅེས་བྱ་བར། དེ་བཞིན་གཤེགས་པ་རྔ་སྒྲ་ཞེས་བྱ་བ་བྱུང་སྟེ" | tenzin-dhonzang:c-1-1 | PASS |
| 20 | "ཇི་སྲིད་བླ་ན་མེད་པའི་བྱང་ཆུབ་མངོན་དུ་བྱས་པ་དེ་སྲིད་དུ་མིང་ལྷ་མོ་སྒྲོལ་མའི་མཚན་འབའ་ཞིག་ཏུ་འགྱུར་བར" | sungrab-tulku:c-2-6 / tenzin-dhonzang:c-1-6 | PASS |
| 21 | "དེ་བཞིན་གཤེགས་པ་ནི་དགའ་བཞི་ཡེ་ཤེས" | palden-sherab:c-3-1-4-3-1 | PASS |
| 22 | "དེ་བཞིན་གཤེགས་པའི་གཙུག་ཏོར་ནི་ཐེག་པ་རིམ་དགུའི་རྩེ་མོ་རྫོགས་ཆེན་ཨ་ཏི་ཡོ་ག་ལྟ་བ" | palden-sherab:c-3-1-4-4-1 | PASS |

**22/22 quotations verified PASS**, each checked directly against its `1-SOURCES/`
commentary file (not the raw claims file's own transcription). No corrections were
needed during this pass — the consolidated page's `**བོད་ཡིག:**` fields were already
faithful copies of the source in every case checked.

Additional mechanical checks performed against the fence body: tsheg present at every
`'''` and `[[` boundary (checked programmatically — 3 bold spans, 6 wikilinks/category,
all pass); `<ref>`/`</ref>`/self-closing `<ref .../>` tags balanced (52 opens = 16 full
definitions + 36 self-closes); no Latin script outside `<ref>` bodies; every `==`/`===`
section (lead through `བསྡུས་དོན།`) carries at least one citation; last three sections in
fixed order `འབྲེལ་ཡོད་ཤོག་ངོས།` → `ལུང་ཁུངས།` → `དཔྱད་གཞིའི་ཡིག་ཆ།`; `<references />` present
(no `{{Reflist}}`); one category `[[རིགས་དབྱེ།:ནང་ཆོས།]]`, on the spec's allowlist.
