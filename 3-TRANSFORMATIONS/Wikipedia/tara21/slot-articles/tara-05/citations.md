...
|---|---|---|---|---|
| taranatha | Tāranātha | c-5-1, c-5-2, c-5-3, c-5-4, c-5-5, c-5-6, c-5-7, c-5-8, c-5-9, c-5-10 | "ཏུཏྟཱ་ར་ནི་གདུང་པ་སྟེ། གཏུམ་མོའི་མེས་གདུང་བའོ།" (c-5-7) | 1-SOURCES/Commentaries/ཕྱག་འཚལ་ཉེར་གཅིག་གི་བསྟོད་པའི་རྣམ་པར་བཤད་པ།.md#^0-20 |
...
| konchok-thabkhe | Konchok Thabkhe | c-2-5-1, c-2-5-3, c-2-5-4 | "གསུམ་དང་བདུན་ནོར་ལུགས་མེད་པས་འགྱུར་སྐྱོན་མིན་ལ།" (c-2-5-4) | 1-SOURCES/Commentaries/ཕྱག་འཚལ་ཉེར་གཅིག་མའི་ཊིཀྐ་འཕགས་མའི་ཞལ་ལུང་ཞེས་བྱ་བ་བཞུགས་སོ།.md#^0-34 |
...
| tenzin-dhonzang | Sermey Tsang Geshe Tenzin Dhonzang | c-4-5-1, c-4-5-4, c-4-5-5, c-4-5-6, c-4-5-7, c-4-5-8 | "མི་གཡེང་བར་ཐུགས་རྗེ་ཆེན་པོས་འགྲོ་བ་རྣམས་རྗེས་སུ་བཟུང་བའི་དོན་ཡིན།" (c-4-5-5); "འཇིག་རྟེན་བདུན་པོ་འདི་ཡལ་བར་མ་འདོར་བ་དང་། མ་སྤངས་པར་བརྩེ་བ་ཆེན་པོས་ཡོངས་སུ་བཟུང་བའི་དོན་རེད།" (c-4-5-5); "ཞིང་འདི་གཞན་གྱིས་སྤངས་དུས་མགོན་ཁྱོད་ཀྱིས། པད་དཀར་ལྟ་བུར་བསྔགས་པའི་དགོས་པའང་ཅི།" (c-4-5-6) | 1-SOURCES/Commentaries/སྒྲོལ་མ་ཉེར་གཅིག་གི་བསྟོད་འགྲེལ་སྙིང་གི་ནོར་བུ.md#^0-140, #^0-143–^0-147 |

16 named refs total (one per commentary in the topic page's `sources:` list, all 16 used). 79 `<ref>` tag occurrences: 16 full definitions + 63 self-closing repeats. 11 direct verbatim quotations, listed in full in the Verification section below.
...
None. Every `commentary:claim-id` attestation drawn on from `2-RAILS/Claims/tara-05.md` for this article resolved cleanly to a claim block in its raw tree-guided file (verbatim བོད་ཡིག, English gloss, `Cite:` block ID, and the raw file's own `author`/`title` frontmatter). Not every one of the ~102 attestations on the consolidated page was used in the article — Rule 3 caps quotations at 1–3 per major section and due weight (Rule 5) means many redundant consensus attestations were represented by 2–4 citing commentaries rather than all sixteen — but no attestation that *was* drawn on failed to resolve.

One resolution note, not a failure: `tsultrim-namdak`'s claims `c-3-8` and `c-2-1-2-1-16/17/18` contain internal `...` elisions in their own **བོད་ཡིག** field (the raw extraction spans multiple non-contiguous source blocks, e.g. `#^0-193, #^0-194, #^0-195, #^0-196, #^0-197`). These claims were used in the article only as paraphrased, attributed content — never quoted directly — since an elided string cannot be verified character-for-character against a single contiguous source passage.
...
Every direct quotation located character-for-character (whitespace-collapsed) in the `1-SOURCES/` file its claim's `Cite:` field names, checked against the raw source file directly (not just the raw claims file) via `grep` at the cited block ID:

| # | Quotation | Commentary / claim | Source block | Result |
|---|---|---|---|---|
...
| 9 | "མི་གཡེང་བར་ཐུགས་རྗེ་ཆེན་པོས་འགྲོ་བ་རྣམས་རྗེས་སུ་བཟུང་བའི་དོན་ཡིན།" | tenzin-dhonzang / c-4-5-5 | ...སྙིང་གི་ནོར་བུ.md#^0-144 | PASS |
...
- V1 — PASS (table above).
- V2 — not applicable in this form: this skill's ref form is the hand-formatted `<AUTHOR>། <TITLE>།` (skill Rule 7), not the `sources.yaml`-resolved form; no `sources.yaml` entry exists yet for this corpus.
- V3 — PASS: `<references />` present, refs exist.
- V4 — PASS: no `{{Reflist}}` anywhere.
- V5 — PASS: 79 `<ref>` tags total (16 full definitions + 63 self-closing), 16 `</ref>` closes, one full definition per named ref, all self-closes reference an already-defined name.
- V6 — PASS: 7 `==` headings.
